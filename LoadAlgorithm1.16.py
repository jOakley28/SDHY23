...
def main(datafile: str):
    ...
    for step_size in tqdm(STEP_SIZES):
        ...
        if fft_sec_calc == True:
            ...
            dt = (
                df.iloc[4, 0] - df.iloc[0, 0]
            ) / 4  # determine dt based on change in time of the first two time steps
            ffilt = type(
                np.ndarray
            )  # create numpy non dementionalized array used to store sets before concatenation
            ...
            for j in range(
                len(fft_df.iloc[:, 2])
            ):  # converts filtered data from ADC to kg
            ...
            if MTS_calc == True:  # does not include MTS plots
                ...
                fftMiniPlot(
                    fft_df[0], fft_df[2], fft_df[1], "kg", 1 / dt, step_size, False
                )
    ...
    if fft_sing_calc == True:
        ...
        ffts_df[1] = fftFilter(
            dt, df[0], df[1], "freq"
        )  # runs fftFilter on entrire data set, 'freq' creates and saves plot of freq vs. power
...
def fftFilter(dt, t, f, freqPlot):
    ...
    )  # only plot the first half of the frequencies
    ...
    if freqPlot == "freq":
        ...
        plt.axhline(
            y=c, color="k", linewidth=0.5
        )  # horizontal and vertical lines indicating data cutoff
        plt.axvline(x=fLBound, color="k", linewidth=0.5)
        plt.axhspan(
            0, c, facecolor="k", alpha=0.7
        )  # horizontal and vertical shading indicating data exclusion
        ...
        plt.savefig(
            f"{saveLocation}/{filename} (General) - Noise Frequency (linear).png"
        )
        plt.cla()
...
def moving_average(array, step_size):
    ...
    for i in tqdm(range(0, len(array), step_size)):
        ...
        averaged_df = pd.concat(
            [averaged_df, pd.DataFrame([[array.iloc[i, 0], average]])]
        )
    input(averaged_df)
...
def fftMiniPlot(t, f, ffilt, yLim, hz, step_size, jr):
    if jr == False:
        ...
        plt.title(
            f"\n{step_size} samples per group\n@{int(hz)}hz - latency* ~{int(step_size/hz)}s",
            loc="right",
            color="g",
        )
        plt.title(f"Circuit {circuitType}", loc="left")
        ...
        plt.ylabel("Load (kg)")
        plt.savefig(
            f"{FilteredAndRaw}/{filename} {step_size}b - FilteredAndRaw load.png"
        )
        plt.cla()
    ...
    if jr == True:
        ...
        plt.ylabel("Load (kg)")
        plt.savefig(
            f"{saveLocation}/{filename} whole data set - FilteredAndRaw load.png"
        )
        plt.cla()
...
def fftMiniPlotMTS(
    ...
    if jr == False:
        ...
        plt.plot(
            MTStime, MTSdata, color="r", linewidth=1.0, alpha=0.7, label=f"Applied MTS"
        )
        if E != "E":
            plt.plot(
                t, f, color="c", linewidth=0.8, label=f"Raw ({int(error_per_r*10)/10}%)"
            )
        else:
        ...
        if E != "E":
            plt.fill_between(
                ebtw_r[0], ebtw_r[1], ebtw_r[2], color="gray", alpha=0.2, label="error"
            )  # TODO
            plt.savefig(
                f"{FilteredAndRaw}/{filename} (General) (with MTS and error) - Raw load.png"
            )
        else:
            plt.title(f"Circuit {circuitType}", loc="left")
            plt.savefig(
                f"{FilteredAndRaw}/{filename} (General) (with MTS) - Raw load.png"
            )
        plt.cla()
        ...
        if E != "E":
            plt.plot(
                ...
                color="k",
                linewidth=1.0,
                label=f"Filtered ({int(error_per_f*10)/10}%)",
            )  # prints filtered data with error label
        else:
            plt.plot(
                t, ffilt, color="k", linewidth=1.0, label="Filtered"
            )  # prints filtered data without error label
        plt.minorticks_on()
        plt.grid(visible=None, which="major", linestyle="-", linewidth="1", axis="both")
        plt.grid(visible=None, which="minor", linestyle="--", linewidth="0.7", axis="y")
        plt.xlim(t[0], t[len(t) - 1])
        ...
        if E != "E":
            plt.fill_between(
                ebtw_r[0], ebtw_r[1], ebtw_r[2], color="gray", alpha=0.2, label="error"
            )  # filles between the MTS reading and the filtred data
            plt.savefig(
                f"{FilteredOnly}/{filename} {step_size} (with MTS and error)a - Filtered load.png"
            )
        else:
            plt.savefig(
                f"{FilteredOnly}/{filename} {step_size} (with MTS)a - Filtered load.png"
            )
        plt.cla()
        ...
        if E != "E":
            ...
            plt.savefig(
                f"{FilteredAndRaw}/{filename} {step_size} (with MTS and error)b - FilteredAndRaw load.png"
            )
        # else:
    ...
    if jr == True:
        ...
        if E != "E":
            plt.savefig(
                f"{saveLocation}/{filename} whole data set (with MTS) - FilteredAndRaw load.png"
            )
        else:
            plt.savefig(
                f"{saveLocation}/{filename} whole data set (with MTS and error) - FilteredAndRaw load.png"
            )
        plt.cla()
...
def getMTSData(MTSdatafile):  # TODO
    ...
    MTS_dt = (
        MTS_df.iloc[1, 0] - MTS_df.iloc[0, 0]
    ) / 2  # determine dt based on change in time of the first two time steps
    ...
    MTS_df[3] = MTS_df[1].apply(
        lambda x: (x * convert)
    )  # converts from applied mts load to local load on the sensor
...
if __name__ == "__main__":
    if True == True:
        ...
        parser.add_argument(
            ...
            "--open",
            dest="open",
            help="1: opens explorer window only, 2: open explor after run ",
            default=0,
            type=int,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "-c", "--circuitType", dest="circuitType", help="circuit type", default="A"
        )
        parser.add_argument(
            "-l", "--letter", dest="letter", help="test letter", default="DEF a"
        )
        parser.add_argument(
            "-d",
            "--description",
            dest="description",
            help="short test description",
            default="- Static Load",
        )
        parser.add_argument(
            "-i",
            "--testInfo",
            dest="testInfo",
            help="longer test description",
            default="DEF: Static loading with user induced noise",
        )
        parser.add_argument(
            "-D", "--datafile", dest="datafile", help="datafile path", default=""
        )
        parser.add_argument(
            "-S",
            "--saveLocation",
            dest="saveLocation",
            help="saveLocation path",
            default="",
        )
        parser.add_argument(
            "-y",
            "--fft_section_bool",
            dest="fft_section_bool",
            help="enables running sectioned sample analysis",
            default=True,
        )
        parser.add_argument(
            "-z",
            "--fft_single_bool",
            dest="fft_single_bool",
            help="enables running single sample analysis",
            default=True,
        )
        parser.add_argument(
            "-m",
            "--mts_bool",
            dest="mts_bool",
            help="enables plotting MTS data with fft data",
            default=True,
        )  # TODO may need to invert bool value
        parser.add_argument(
            "-M",
            "--mts_datafile",
            dest="mts_datafile",
            help="MTS datafile path",
            default="",
        )
        ...
        description = (str(args.description)[1:-1]).replace(
            "'", ""
        )  # short description of test (for file names)
        testInfo = (str(args.testInfo)[1:-1]).replace(
            ",", ""
        )  # additional information (for charts)
        ...
        MTSfilename = pathlib.Path(
            args.mts_datafile
        )  # location to pull MTS datafile from

        fft_sec_calc = args.fft_section_bool
        fft_sing_calc = args.fft_single_bool
        # determines which filters are run
    ...
    if MTS_calc != True:
        os.rename(
            f"{saveLocation}/{MTSfilename.parts[-1]}",
            f"{saveLocation}{test_key} MTS.txt",
        )
    ...
    with open(f"{saveLocation}info.txt", "w") as f:
        ...
        if MTS_calc == True:
            f.write(
                f"python3 LoadAlgorithm1.16.py -c {args.circuitType} -l {args.letter} -d '{args.description}' -i '{args.testInfo}' -D '{finalDataFileName}' -S '{args.saveLocation}' -s {stepHolder}\n"
            )
        else:
            f.write(
                f"python3 LoadAlgorithm1.16.py -c {args.circuitType} -l {args.letter} -d '{args.description}' -i '{args.testInfo}' -D '{finalDataFileName}' -S '{args.saveLocation}' -m 1 -M {newMTSName} -s {stepHolder}\n"
            )
