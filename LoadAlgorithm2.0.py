...
def errorBetweenCurves(sampleA, sampleB):  #determine the error (both instantenous and average)\
    c = []
    errorC = []    
...
def stretch_data(sampleAx: np.array,sampleAy: np.array,sampleBx: np.array,sampleBy: np.array):    #makes both arrays the same length (used for error calculation)
...
def lin_interpolate(array: np.ndarray, new_len: int) -> np.ndarray: #used in stretch data
    la = len(array)
...
def plot_two(ard, mts, ard_freq, avgArdError, ardError, errorBtw, address):   #2 col df where 0: time, 1: data
    ...
    plt.fill_between(errorBtw[0], errorBtw[1], errorBtw[2], color='gray', alpha=0.2, label='error')
